...
function initPainter() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // 创建SVG图层防止高亮时线路遮挡站点和图标
            var $gBg = $.svg('g').attr('id', 'g-bg').appendTo('#g-box');
            var $gLines = $.svg('g').attr('id', 'g-lines').appendTo('#g-box');
            var $gStations = $.svg('g').attr('id', 'g-stations').appendTo('#g-box');
        ...
        for (var i = 0; i < ls.length; i++) {
            ...
            for (var n = 0; n < $(ls[i]).attr("lp").split(";").length; n++) {
                if ($(ls[i]).attr("lp").split(";")[n]) {
                    ...
                    var rect = $.svg('rect').appendTo('#g-bg')
                    rect.attr({
                        x: lp[0] * 1,
                        y: lp[1] * 1,
                        width: lp[2] * 1,
                    ...
                    var text = $.svg('text').appendTo('#g-bg')
                    text.attr({
                        x: lp[0] * 1 + lp[2] / 2,
                        y: lp[1] * 1 + lp[3] / 3 * 2,
                        'data-line': lcode,
                        'class': 'subway-name subway-line line-' + lcode
            ...
            for (var j = 0; j < ps.length; j++) {
                ...
                if (thisP.attr("arc")) {
                    var path = $.svg('path').appendTo('#g-lines')
                        path.attr({
                ...
                var line = $.svg('line').appendTo('#g-lines')
                line.attr({
                    x1: thisP.attr("x") * 1,
                    y1: thisP.attr("y") * 1,
            ...
            for (var j = 0; j < ps.length; j++) {
                ...
                var text = $.svg('text').appendTo('#g-stations')
                text.attr({
                    "font-size": 12,
                ...
                if (thisP.attr("iu") === "false") {
                    ...
                    var text1 = $.svg('text').appendTo('#g-stations')
                    text1.attr({
                        "font-size": 12,
                        x: thisP.attr("x") * 1 + thisP.attr("rx") * 1,
                ...
                if (thisP.attr("ex") === "true") {
                    var image = $.svg('image').appendTo('#g-stations')
                    image.attr({
                        width: "14",
                        height: "14",
                        x: thisP.attr("x") - 7,
                    ...
                    var circle = $.svg('circle').appendTo('#g-stations')
                    circle.attr({
                        r: 4,
                        cx: thisP.attr("x") * 1,
                        cy: thisP.attr("y") * 1,
                        ...
                        .attr({ 'data-line': lcode, 'class': 'subway-line station-dot line-' + lcode })
                    if (thisP.attr("iu") === "false") {
        ...
        // 移除 viewBox，交由 svg-pan-zoom 通过 transform 控制，以确保缩放和平移计算的准确性
        try {
        ...
        try {
            ...
            // 计算"天安门西"(952, 666)到屏幕中心的平移量
            var centerX = sizes.width / 2;
            var centerY = sizes.height / 2;
            
            panzoom.pan({ 
                x: centerX - 952 * currentZoom - 150, 
                y: centerY - 666 * currentZoom + 20
            });
